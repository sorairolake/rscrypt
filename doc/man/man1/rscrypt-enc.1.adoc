...
_OUTFILE_::
  ...
  Output file.
  If _OUTFILE_ is not specified, the result will be write to stdout.
...
*-M*, *--max-memory* _BYTES_::
  ...
  _BYTES_ can also be provided as bytes with the unit symbol (such as MiB and
  GB).
  _BYTES_ should be between 1 MiB and 16 EiB.

*-m*, *--max-memory-fraction* _RATE_::

  Use at most the specified fraction of the available RAM to compute the
  derived key.
  _RATE_ can be provided as a decimal or a fraction.
  _RATE_ should be greater than 0 and less than or equal to *0.5*.
  Default is 0.125.
...
*rscrypt*(1), *rscrypt-dec*(1), *rscrypt-info*(1), *scrypt*(1)
